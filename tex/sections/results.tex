The code developed during this project is openly available at \url{github.com/hkve/Lipkin-Model-FYS5419}. Viewer discretion is advised.
...
\noindent The results of using VQE on the simple $2 \times 2$ Hamiltonian is presented in \cref{fig:toy_1}. It is the lower section of \cref{fig:res:changing_character} extracted for comparison with VQE. It correctly predicts the ground state energy for this simple Hamiltonian, verifying the precision of the method for basic usage. This is an essential step in developing the system for more complex Hamiltonians and systems, which follows.

\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{figs/toy1.pdf}
    \caption{Showing ground state energy of the $2\times 2$ Hamiltonian, calculated using diagonalization and the VQE for $\lambda \in [0,1]$.}\label[fig]{fig:toy_1}
\end{figure}

The slightly larger $4 \times 4$ Hamiltonian ground state energy calculated using VQE is shown in \cref{fig:res:toy_2}. Again we see a good correspondence between the diagonalization and our VQE calculations.  
\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{figs/toy2.pdf}
    \caption{Showing ground state energy of the $4\times 4$ Hamiltonian, calculated using diagonalization and the VQE for $\lambda \in [0,1]$.}
    \label[fig]{fig:res:toy_2}
\end{figure}
