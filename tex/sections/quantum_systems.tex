...
\begin{align}
    ...
    \end{split}
\end{align}
Additionally if we set $W = 0$, further reductions of the problem can be performed. Looking back at \cref{eq:theo:lipkin_quasispin_hamiltonian}, we note that if only $H_0$ and $H_1$ are included,  spins differing by $\pm2$ are the only possible interaction couplings. Since $H_0$ is diagonal and single particle energies are degenerate, it does not break the pairing symmetry. Instead of the $2J+1$ spin projections, we simply have $J+1$ relevant states.From \citep{hlatshwayoSimulatingExcitedStates2022} we have the two-qbit Hamiltonian for $N=4$ as
\begin{align}
